class Ascendig:
    def selection_sort(self, A, entry):
        n = len(A)       # length            
    ...
    def quick_Sort(self, A, col, l, h):
        # base case if array length is greater than 1 
        if (l < h):
            # partition index
            pi = self.partition(A, l, h, col)
            # recursions for sublists
            self.quick_Sort(A, col, l, pi - 1)
            self.quick_Sort(A, col, pi + 1, h)


    def partition (self, A, l, h, col):
    ...
    def Merge_Sort(self, A, col, l, r):
        # BASE CASE
        if l < r:  
            m = int(l + (r-l)/2)
            self.Merge_Sort(A, col, l, m)
            self.Merge_Sort(A, col, m+1, r)
            self.Merge(A, l, m, r, col)


    def Merge(self, A, l, m, r, col): 
